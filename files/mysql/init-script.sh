...
CREATE FUNCTION getNbDaysCurrentPeriod() RETURNS INT DETERMINISTIC
    BEGIN
        RETURN DAYOFMONTH(LAST_DAY(DATE_SUB(DATE_SUB(NOW(), INTERVAL ${DAYS_OFFSET} DAY), INTERVAL 1 MONTH)));
    END;;
